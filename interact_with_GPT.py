...
def analysis_exercise(title):
    ...
    an_result["如图，在等腰△ABC中，AB=AC，过C作CD⊥AB于D. 求证：BC^2=2*BD*BA."] = '''{
                             "已知条件": ["triangle(A,B,C)", "AB=AC", "lieson(D,AB)", "perpendicular(CD,AB)"],
                             "待求结论": ["BC^2=2*BD*BA"]
                           } '''
    ...
    an_result["如图，已知PQ平行于矩形ABCD的边AD.求证: AP^2+CQ^2=PB^2+DQ^2."] = '''{
                                                 "已知条件": ["rectangle(A,B,C,D)", "parallel(PQ,AD)"],
                                                 "待求结论": ["AP^2+CQ^2=PB^2+DQ^2"]
                                               } '''
    an_result["如图，ABCD是圆内接四边形，对角线AC交BD于E，BE=ED.求证：AB^2+BC^2+CD^2+DA^2=2AC^2."] = '''{
    ...
    api_key = "a478c348d9f640a7808e322f3edfc285"
...
def polish_exercise(title):
    ...
    headers = {
        "Content-Type": "application/json",
        "api-key": "a478c348d9f640a7808e322f3edfc285"
    }
    # 请求的数据
    data = {
        "messages": messages,
...
if __name__ == "__main__":
    ...
    exercise = "如图，在矩形ABCD中，点E是AD的中点，过A、E、C三点的圆交直线CD于另一点F.求证:AF⊥BE."
