        ...
        if slove_result[0] == 0:  # 恒等式求解成功,拼接恒等式
            ...
            subexpression_list.append(
                [
                    ...
                    con_expression[1], [], con_expression[1]
                ]
            )
                ...
                if v != 0:
                    subexpression_list.append(
                        [
                            ...
                            hyp_expression[k][1], [], hyp_expression[k][1]
                        ]
                    )
            ...
            for i in range(0, len(subexpression_list)):
                for point in point_replace_use.keys():
                    if point in subexpression_list[i][1]:  # 衍生点必须要这样统一替换,不然计算的时候恒等式等于0
                        ...
                        subexpression_list[i][3].append('{}={}'.format(point, point_replace_use[point][0]))  # 形成衍生点替换关系
...
if __name__ == "__main__":
    ...
    hypothesis = ['triangle(A,B,C)', 'perpendicular(AB,AC)', 'lieson(P,BC)', 'lieson(Q,BC)', 'BA=BQ', 'CA=CP']
    conclusion =['PQ^2=2*BP*QC']
